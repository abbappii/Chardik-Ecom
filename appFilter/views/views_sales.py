...
class passParams(APIView):
    def get(self,request, *args, **kwargs):
        ...
        if monthly: 
            query = query.filter(created_at__gte = now - timedelta(day=30)).aggregate(monthly_sales =Sum('total'))
        if half_yearly:
            query = query.filter(created_at__gte = now - timedelta(day = 6 * 365 /12 )).aggregate(half_yearly =Sum('total'))

        if yearly:
            query = query.filter(created_at__gte = now - timedelta(day = 365 )).aggregate(yearly =Sum('total'))
